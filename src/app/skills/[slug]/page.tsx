...
export default async function SkillDetailPage({ params }: PageProps) {
  ...
  return (
    <main className="notebook-lines min-h-screen">
          ...
          <span className="text-foreground-secondary">
            ...
            {skill.authorUrl ? (
              <a
                ...
                target="_blank"
                rel="noopener"
                className="font-medium text-foreground hover:text-amber-700 underline underline-offset-2"
              >
                {skill.authorName}
              </a>
            ) : (
        ...
        {skill.llmTags.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mb-4">
            {skill.llmTags.map((tag) => (
              <span
                key={tag}
                className="inline-flex items-center px-2 py-0.5 text-xs border border-sketch/30 text-foreground-secondary rounded-[var(--radius-sm)]"
              >
                {tag}
              </span>
            ))}
      ...
      {/* Reviews */}
      <section className="border-t border-warm-200 pt-8 mb-8">
        <ReviewFeed
      ...
      {/* Review Form */}
      <section className="border-t border-warm-200 pt-8 mb-12">
